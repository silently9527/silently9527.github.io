...
其中的RingBufferDescriptor.TRAILER_LENGTH 以及 为何write的数据需要额外写 8 字节在后续的源码分析中解答。
...
填充消息的类型PADDING_MSG_TYPE_ID(=-1), 填充的消息长度等于toBufferEndLength，在读取的时候就使用这两个值跳过填充类型的消息。
由于是从数组的开头写入，所以还需要判断需要申请的空间大小与headIndex比较，如果可写入的空间不够就返回 -2
...
同write方法一样先调用claimCapacity方法去申请写入的空间
空间申请成功后 只写入消息的类型与消息长度的负数 -recordLength
返回了实际消息的写入位置
...
解读出消息后 调用MessageHandler.onMessage处理消息
在读取数据的时候可能会遇到填充的数据 msgType=PADDING_MSG_TYPE_ID, 直接跳过这种类型的消息
循环结束条件：读取的总字节数>=允许读取的连续空间contiguousBlockLength=capacity - headIndex 或者已经读取的消息数 > 设置的最大值
finally中更新headPositionIndex到最新位置
try {
