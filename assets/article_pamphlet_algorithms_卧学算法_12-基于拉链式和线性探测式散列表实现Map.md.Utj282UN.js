...
<span class="line"><span>}</span></span></code></pre></div><h4 id="double、float" tabindex="-1">Double、Float <a class="header-anchor" href="#double、float" aria-label="Permalink to &quot;Double、Float&quot;">​</a></h4><p>浮点类型的键java中hashCode的实现是将键表示为二进制</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>public static int hashCode(float value) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h4 id="插入" tabindex="-1">插入 <a class="header-anchor" href="#插入" aria-label="Permalink to &quot;插入&quot;">​</a></h4><ol><li>当插入键的位置超过了数组的大小，就需要回到数组的开始位置继续查找，直到找到一个位置为null的才结束；<code>index = (index + 1) % capacity</code></li><li>当数组已存放的容量超过了数组总容量的一半，就需要扩容到原来的2倍</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public void put(K key, V value) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h4 id="查询" tabindex="-1">查询 <a class="header-anchor" href="#查询" aria-label="Permalink to &quot;查询&quot;">​</a></h4><p>线性探测散列表中实现查询的思路：根据待查询key的hash函数计算出索引的位置，然后开始判断当前位置上的key是否和待查询key相等，如果相等那么就直接返回value，如果不相等那么就继续查找下一个索引直到遇到某个位置是null才结束，表示查询的key不存在；</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public V get(K key) {</span></span>
<span class="line"><span>    if (Objects.isNull(key)) {</span></span>
<span class="line"><span>        throw new IllegalArgumentException(&quot;Key can not null&quot;);</span></span>
<span class="line"><span>    }</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h4 id="删除元素" tabindex="-1">删除元素 <a class="header-anchor" href="#删除元素" aria-label="Permalink to &quot;删除元素&quot;">​</a></h4><p>线性探测式的删除稍微麻烦一些，首先需要查找出待删除的元素位置，删除元素之后需要把当前索引之后的连续位置上的元素重新插入；因为是否有空位对于线性探测式散列表的查找至关重要；例如：5-&gt;7-&gt;9，删除了7之后，如果不重新插入7的位置就是空位，那么get方法将无法查询到9这个元素；</p><p>每次删除之后都需要检测一次数组中实际元素的个数，如果与数组的容量相差较大，就可以进行缩容操作；</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public void delete(K key) {</span></span>
<span class="line"><span>    if (Objects.isNull(key)) {</span></span>
<span class="line"><span>        throw new IllegalArgumentException(&quot;Key can not null&quot;);</span></span>
<span class="line"><span>    }</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div>`,53);function f(a,_,x,C,q,M){const l=t,i=o("ClientOnly");return p(),r("div",null,[m,h(i,null,{default:d(()=>{var s,n;return[(((s=a.$frontmatter)==null?void 0:s.aside)??!0)&&(((n=a.$frontmatter)==null?void 0:n.showArticleMetadata)??!0)?(p(),b(l,{key:0,article:a.$frontmatter},null,8,["article"])):v("",!0)]}),_:1}),y])}const V=c(k,[["render",f]]);export{I as __pageData,V as default};
