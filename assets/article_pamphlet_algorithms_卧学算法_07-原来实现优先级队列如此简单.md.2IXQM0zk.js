import{_ as t}from"./chunks/ArticleMetadata.4VTDl_qq.js";import{_ as c,D as r,o as p,c as o,I as u,w as d,k as e,a as h,R as m,b,e as g}from"./chunks/framework.412fhWlr.js";import"./chunks/md5.6OGdLw3O.js";const A=JSON.parse('{"title":"原来实现优先级队列如此简单","description":"","frontmatter":{"title":"原来实现优先级队列如此简单","author":"Herman","date":"2021/08/14 13:58","categories":["算法"],"tags":["数据结构","二叉堆"]},"headers":[],"relativePath":"article/pamphlet/algorithms/卧学算法/07-原来实现优先级队列如此简单.md","filePath":"article/pamphlet/algorithms/卧学算法/07-原来实现优先级队列如此简单.md","lastUpdated":1723379826000}'),q={name:"article/pamphlet/algorithms/卧学算法/07-原来实现优先级队列如此简单.md"},k=e("h1",{id:"原来实现优先级队列如此简单",tabindex:"-1"},[h("原来实现优先级队列如此简单 "),e("a",{class:"header-anchor",href:"#原来实现优先级队列如此简单","aria-label":'Permalink to "原来实现优先级队列如此简单"'},"​")],-1),v=m(`<h2 id="前言" tabindex="-1">前言 <a class="header-anchor" href="#前言" aria-label="Permalink to &quot;前言&quot;">​</a></h2><p>假如你设计的事件系统中有很多的事件，每个事件都定义了不同的权重值，系统需要优先处理权重较高的事件，这里你就需要使用到优先级队列，本篇我们一起来学习实现优先级队列的常用方式</p><h2 id="队列api定义" tabindex="-1">队列API定义 <a class="header-anchor" href="#队列api定义" aria-label="Permalink to &quot;队列API定义&quot;">​</a></h2><p>在实现之前，首先我们需要先定义出优先级队的API，优先级队列是一种抽象的数据结构，我们依然可以基于前面我们使用到的队列API来修改；需要了解之前的队列的实现可以查看《面试的季节到了，老哥确定不来复习下数据结构吗》</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>public interface Queue&lt;T&gt; extends Iterable&lt;T&gt; {</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>    //省略其他函数</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>enqueue时间复杂度是O(n)，dequeue时间复杂度是O(1)</p><h4 id="基于链表实现" tabindex="-1">基于链表实现 <a class="header-anchor" href="#基于链表实现" aria-label="Permalink to &quot;基于链表实现&quot;">​</a></h4><p>基于链表的实现与上面的类似，有兴趣的可以自己实现</p><blockquote><p>在《面试的季节到了，老哥确定不来复习下数据结构吗》中我们实现的栈和队列的操作都能够在常数时间内完成，但是优先级队列从上面的实现过程，我们发现初级版本的实现插入或删除最大值的操作最糟糕的情况会是线性时间。</p></blockquote><h2 id="二叉堆实现" tabindex="-1">二叉堆实现 <a class="header-anchor" href="#二叉堆实现" aria-label="Permalink to &quot;二叉堆实现&quot;">​</a></h2><h4 id="二叉堆的定义" tabindex="-1">二叉堆的定义 <a class="header-anchor" href="#二叉堆的定义" aria-label="Permalink to &quot;二叉堆的定义&quot;">​</a></h4><p>在二叉堆中，每个节点都将大于等于它的子节点，也成为堆有序；其中根节点是最大的节点。</p><p>二叉堆的表示：</p><p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b98dc1760a6e407892ac6af85cc436d2~tplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p>重点： 在一个二叉堆中，位置k节点的父节点的位置为<code>k/2</code>,它的两个子节点的位置为<code>2k</code>和<code>2k+1</code>; 基于这点，我们可以用数组来表示二叉堆，通过移动数组的下标来找到节点父节点和子节点</p><p>在元素进行插入和删除操作的过程中，会破坏堆有序，所以我们需要做一些操作来保证堆再次有序；主要有两种情况，当某个节点的优先级上升，我们需要<strong>由下向上恢复堆有序（下沉）</strong>；当某个节点优先级下降，我们需要<strong>由上向下恢复堆有序（上浮）</strong></p><h4 id="由上向下恢复堆有序-上浮" tabindex="-1">由上向下恢复堆有序（上浮） <a class="header-anchor" href="#由上向下恢复堆有序-上浮" aria-label="Permalink to &quot;由上向下恢复堆有序（上浮）&quot;">​</a></h4><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>private void swim(int k) {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h4 id="二叉堆实现优先级队列" tabindex="-1">二叉堆实现优先级队列 <a class="header-anchor" href="#二叉堆实现优先级队列" aria-label="Permalink to &quot;二叉堆实现优先级队列&quot;">​</a></h4><ul><li>入队操作：将新的元素添加到数组末尾，让新元素上浮到适合位置，增加堆的大小</li><li>出队操作：将最大的根节点删除，然后把最后一个元素放入到顶端，下层顶端元素到合适位置，减小堆大小</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>public class BinaryHeapPriorityQueue&lt;T&gt; extends AbstractQueue&lt;T&gt; {</span></span>
<span class="line"><span>    private T[] queue;</span></span>
<span class="line"><span>    private int size;</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    </span></span>
<span class="line"><span>     //省略其他函数</span></span>
<span class="line"><span>}</span></span></code></pre></div><blockquote><p>注意：</p><p>由于我们为了方便计算父节点和子节点的索引位置，所以数组中的第一个位置是不会使用的；可以自己思考下使用第一个位置，那么子节点和父节点的位置应该如何计算？</p><p>基于堆的实现，入队和出队的时间复杂对都是logN，解决了初级版本实现的问题。</p><p>数组大小动态扩容和缩容依然可以参考之前栈的实现方式</p></blockquote>`,36);function _(a,T,f,x,y,P){const l=t,i=r("ClientOnly");return p(),o("div",null,[k,u(i,null,{default:d(()=>{var s,n;return[(((s=a.$frontmatter)==null?void 0:s.aside)??!0)&&(((n=a.$frontmatter)==null?void 0:n.showArticleMetadata)??!0)?(p(),b(l,{key:0,article:a.$frontmatter},null,8,["article"])):g("",!0)]}),_:1}),v])}const Q=c(q,[["render",_]]);export{A as __pageData,Q as default};
