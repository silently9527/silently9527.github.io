...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>算法分析： 看起来quick-union算法比quick-find算法更快，因为union不需要为每对输入遍历整个数组， 考虑最佳情况下，find方法只需要访问一次数组就可以得到根触点，那么union方法的时间复杂度O(n)； 考虑到最糟糕的输入情况，如下图：</li></ul><p><img src="https://raw.githubusercontent.com/silently9527/images/main/17496e49c2a94903a42501b0d99e1ba2~tplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p>find方法需要访问数组n-1次，那么union方法的时间复杂度是O(n²)</p><h2 id="加权quick-union算法" tabindex="-1">加权quick-union算法 <a class="header-anchor" href="#加权quick-union算法" aria-label="Permalink to &quot;加权quick-union算法&quot;">​</a></h2><p>为了保证quick-union算法最糟糕的情况不在出现，我需要记录每一个树的大小，在进行分量归并操作时总是把小的树连接到大的树上，这种算法构造出来树的高度会远远小于未加权版本所构造的树高度。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>public class WeightedQuickUnionImpl extends AbstractUF {</span></span>
