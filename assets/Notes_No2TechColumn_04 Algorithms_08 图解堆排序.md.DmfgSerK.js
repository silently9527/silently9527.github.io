import{_ as a,c as s,o as n,aa as e}from"./chunks/framework.DOnJscRE.js";const b=JSON.parse('{"title":"08 图解堆排序","description":"","frontmatter":{"title":"08 图解堆排序","author":"Herman","updateTime":"2024-07-16 21:34","desc":"图解堆排序","categories":"算法","tags":"数据结构/排序算法/堆排序","outline":"deep"},"headers":[],"relativePath":"Notes/No2TechColumn/04 Algorithms/08 图解堆排序.md","filePath":"Notes/No2TechColumn/04 Algorithms/08 图解堆排序.md","lastUpdated":1724595807000}'),p={name:"Notes/No2TechColumn/04 Algorithms/08 图解堆排序.md"},i=e(`<h1 id="图解堆排序" tabindex="-1">图解堆排序 <a class="header-anchor" href="#图解堆排序" aria-label="Permalink to &quot;图解堆排序&quot;">​</a></h1><h2 id="前言" tabindex="-1">前言 <a class="header-anchor" href="#前言" aria-label="Permalink to &quot;前言&quot;">​</a></h2><p>在上一篇中我们一起使用二叉堆实现了优先级队列，假如我们从构建好的优先级队列中持续调用删除最小（或者最大），把结果输出到另一个数组中，那么就可以把数组的所有元素进行排序，这就是本篇我们需要学习的堆排序。在看本篇之前需要先看下前一篇《原来实现优先级队列如此简单》</p><p>堆排序的过程主要有两个阶段：</p><ul><li>把原始数据构造成一个有序堆（构造堆）</li><li>从堆中按照递减顺序取出所有元素就可以得到排序结果（下沉排序）</li></ul><p>开始之前，我们需要把上一篇中的sink()方法修改一下；</p><p>保证我们在进行排序的时候直接使用原始的数组，无需建立一个辅助数组浪费空间；由于我们需要动态的缩小堆的大小，需要把size通过参数传入；</p><p>其次我们数组的下标是从0开始的，与之前的优先级排序有些差别，对于k节点的左边节点下标是2k+1，右边下标是2k+2</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>private void sink(Comparable[] queue, int k, int size) {</span></span>
