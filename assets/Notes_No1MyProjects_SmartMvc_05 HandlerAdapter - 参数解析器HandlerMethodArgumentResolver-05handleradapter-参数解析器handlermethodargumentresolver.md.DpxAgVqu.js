import{_ as s,c as n,o as a,aa as e}from"./chunks/framework.d_Ke7vMG.js";const h=JSON.parse('{"title":"05 HandlerAdapter参数解析器","description":"","frontmatter":{"title":"05 HandlerAdapter参数解析器","author":"Herman","updateTime":"2021-08-14 13:41","desc":"深入解析SpringMVC核心原理：从手写简易版MVC框架开始(SmartMvc)","categories":"MyProjects","tags":"SpringMvc/MVC","outline":"deep"},"headers":[],"relativePath":"Notes/No1MyProjects/SmartMvc/05 HandlerAdapter - 参数解析器HandlerMethodArgumentResolver-05handleradapter-参数解析器handlermethodargumentresolver.md","filePath":"Notes/No1MyProjects/SmartMvc/05 HandlerAdapter - 参数解析器HandlerMethodArgumentResolver-05handleradapter-参数解析器handlermethodargumentresolver.md","lastUpdated":1723563552000}'),p={name:"Notes/No1MyProjects/SmartMvc/05 HandlerAdapter - 参数解析器HandlerMethodArgumentResolver-05handleradapter-参数解析器handlermethodargumentresolver.md"},t=e(`<p>本节我们将开始开发在HandlerAdapter中需要使用到的组件<code>HandlerMethodArgumentResolver</code>，原本计划是在本节之前先聊聊SpringMVC中的数据绑定，毕竟数据绑定在SpringMVC，甚至是Spring框架中都有重要的地位，后来发现想要深入源码讲清楚数据绑定有些费劲，对于理解SpringMVC的核心原理无太多作用，所以决定在最后做大概三节番外篇来聊聊数据绑定。</p><p>在我们享受SpringMVC给我带来的便利的时候，不知道大家有没有想过，Controller中方法的参数是如何完成自动注入的，在添加上注解<code>@PathVariable</code>、<code>@RequestParam</code>、<code>@RequestBody</code>就能够把请求中的参数主动注入，甚至在方法参数任意位置写<code>HttpServletRequest</code>、<code>HttpSession</code>等类型的参数，它自动就有值了便可直接使用；现在我们就开始来逐步实现这个功能，本节主要实现参数的解析。首先还是需要先看看类图</p><p><img src="https://raw.githubusercontent.com/silently9527/images/main/819027688-5fcba2e425b21_articlex" alt=""></p><p>本节我们简单实现解析<code>HttpServletRequest</code>、<code>HttpServletResponse</code>、<code>Model</code>以及注解<code>@RequestParam</code>、<code>@RequestBody</code>的功能，SpringMVC提供其他参数解析器实现类似，可以自行查看SpringMVC源码</p><h4 id="_7-1-开发步骤讲解" tabindex="-1">7.1 开发步骤讲解 <a class="header-anchor" href="#_7-1-开发步骤讲解" aria-label="Permalink to &quot;7.1 开发步骤讲解&quot;">​</a></h4><blockquote><p>本节源代码的分支：handlerAdpater-argumentResolver</p></blockquote><h5 id="modelandviewcontainer" tabindex="-1">ModelAndViewContainer <a class="header-anchor" href="#modelandviewcontainer" aria-label="Permalink to &quot;ModelAndViewContainer&quot;">​</a></h5><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ModelAndViewContainer {</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    });</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>该单元测试中有两点说明一下：</p><ol><li><code>DefaultFormattingConversionService</code>: 该类是Spring中的一个数据转换器服务，默认已经添加了很多转换器，这里我们还设置了日期转换的格式<code>yyyy-MM-dd HH:mm:ss</code></li><li><code>DefaultParameterNameDiscoverer</code>: 该类是用于查找参数名的类，因为一般来说，通过反射是很难获得参数名的，只能取到参数类型，因为在编译时，参数名有可能是会改变的，所以需要这样一个类，Spring已经实现了多种解析，我们这里直接引用就行</li></ol><p>最后打印出解析出来的参数名字、值、类型</p><p><img src="https://raw.githubusercontent.com/silently9527/images/main/276501005-5fcb7c433e81d_articlex" alt=""></p><ol start="2"><li>编写单元测试验证用例2</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Test</span></span>
