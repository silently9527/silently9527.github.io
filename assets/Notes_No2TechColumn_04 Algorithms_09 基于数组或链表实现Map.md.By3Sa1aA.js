import{_ as s,c as n,o as a,aa as p}from"./chunks/framework.DlhgB44u.js";const g=JSON.parse('{"title":"09 基于数组或链表实现Map","description":"","frontmatter":{"title":"09 基于数组或链表实现Map","author":"Herman","updateTime":"2024-07-16 21:34","desc":"基于数组或链表实现Map","categories":"算法","tags":"数据结构/排序算法/堆排序","outline":"deep"},"headers":[],"relativePath":"Notes/No2TechColumn/04 Algorithms/09 基于数组或链表实现Map.md","filePath":"Notes/No2TechColumn/04 Algorithms/09 基于数组或链表实现Map.md","lastUpdated":1723563552000}'),e={name:"Notes/No2TechColumn/04 Algorithms/09 基于数组或链表实现Map.md"},l=p(`<h1 id="基于数组或链表实现map" tabindex="-1">基于数组或链表实现Map <a class="header-anchor" href="#基于数组或链表实现map" aria-label="Permalink to &quot;基于数组或链表实现Map&quot;">​</a></h1><h2 id="前言" tabindex="-1">前言 <a class="header-anchor" href="#前言" aria-label="Permalink to &quot;前言&quot;">​</a></h2><p>JAVA中的Map主要就是将一个键和一个值联系起来。虽然JAVA中已经提供了很多Map的实现，为了学习并掌握常用的数据结构，从本篇开始我将自己实现Map的功能，本篇主要是通过数组和链表两种方式实现，之后提供二叉树，红黑树，散列表的版本实现。通过自己手写各个版本的Map实现，掌握每种数据结构的优缺点，可以在实际的工作中根据需要选择适合的Map。</p><h2 id="map-api的定义" tabindex="-1">Map API的定义 <a class="header-anchor" href="#map-api的定义" aria-label="Permalink to &quot;Map API的定义&quot;">​</a></h2><p>在开始之前，我们需要先定义出Map的接口定义，后续的版本都会基于此接口实现</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface Map&lt;K, V&gt; {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这个接口是最简单的一个Map定义，相信这些方法对于java程序员来说不会陌生；</p><h2 id="基于链表实现map" tabindex="-1">基于链表实现Map <a class="header-anchor" href="#基于链表实现map" aria-label="Permalink to &quot;基于链表实现Map&quot;">​</a></h2><ol><li>基于链表实现首先我们需要定义一个Node节点，表示我们需要存储的key、vlaue</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>class Node {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><ol start="2"><li>get方法的实现思路是遍历链表，然后比较每个Node中的key是否相等，如果相等就返回value，否则返回null</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ol start="3"><li>put方法的实现思路也是遍历链表，然后比较每个Node的key值是否相等，如果相等那么覆盖掉value，如果未查找到有key相等的node，那么就新建一个Node放到链表的开头</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ol start="4"><li>delete方法实现同样也需要遍历链表，因为我们的是单向链表，删除某个节点有两种思路，第一种，在遍历链表的时候记录下当前节点的上一个节点，把上一个节点的next指向当前节点next;第二种，当遍历到需要删除的节点时，把需要删除节点的next的key、value完全复制到需要删除的节点，把next指针指向next.next，比如：first - &gt; A -&gt; B -&gt; C -&gt; D -&gt; E -&gt; F -&gt; G -&gt; NULL,要删除 C 节点，就把D节点完全复制到c中，然后C -&gt; E，变相删除了C</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><blockquote><p>分析上面基于链表实现的map，每次的put、get、delete都需要遍历整个链表，非常的低效，无法处理大量的数据，时间复杂度为O(N)</p></blockquote><h2 id="基于数组实现map" tabindex="-1">基于数组实现Map <a class="header-anchor" href="#基于数组实现map" aria-label="Permalink to &quot;基于数组实现Map&quot;">​</a></h2><p>基于链表的实现非常低效，因为每次操作都需要遍历链表，假如我们的数据是有序的，那么查找的时候我们可以使用二分查找法，那么get方法会加快很多</p><p>为了体现出我们的Map是有序的，我们需要重新定义一个有序的Map</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SortedMap&lt;K extends Comparable&lt;K&gt;, V&gt; extends Map&lt;K, V&gt; {</span></span>
<span class="line"><span>    int rank(K key);</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>该定义要求key必须实现接口<code>Comparable</code>，rank方法如果key值存在就返回对应在数组中的下标，如果不存在就返回小于key键的数量</p><ul><li>在基于数组的实现中，我们会定义两个数组变量分部存放keys、values；</li><li>rank方法的实现：由于我们整个数组都是有序的，我们可以二分查找法（可以查看《老哥是时候来复习下数据结构与算法了》），如果存在就返回所在数组的下表，如果不存在就返回0</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ul><li>get方法实现：基于rank方法，判断返回的keys[index]与key进行比较，如果相等返回values[index]，不相等就返回null</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public V get(K key) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ul><li>put方法实现：基于rank方法，判断返回的keys[index]与key进行比较，如果相等直接修改values[index]的值，如果不相等表示不存在该key，需要插入并且移动数组</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public void put(K key, V value) {</span></span>
<span class="line"><span>    int index = this.rank(key);</span></span>
<span class="line"><span>    if (index &lt; size &amp;&amp; key.compareTo(keys[index]) == 0) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ul><li>delete方法实现：通过rank方法判断该key是否存在，如果不存在就直接返回，如果存在需要移动数组</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Override</span></span>
<span class="line"><span>public void delete(K key) {</span></span>
<span class="line"><span>    int index = this.rank(key);</span></span>
...
<span class="line"><span>    size--;</span></span>
<span class="line"><span>}</span></span></code></pre></div><blockquote><p>基于数组实现的Map，虽然get方法采用的二分查找法，很快O(logN)，但是在处理大量数据的情况下效率依然很低，因为put方法还是太慢；下篇我们将基于二叉树来实现Map，继续改进提升效率</p></blockquote>`,31),i=[l];function t(c,o,d,r,u,h){return a(),n("div",null,i)}const v=s(e,[["render",t]]);export{g as __pageData,v as default};
