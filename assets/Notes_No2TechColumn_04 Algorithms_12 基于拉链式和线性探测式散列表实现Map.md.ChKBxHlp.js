...
<span class="line"><span>}</span></span></code></pre></div><h4 id="软缓存" tabindex="-1">软缓存 <a class="header-anchor" href="#软缓存" aria-label="Permalink to &quot;软缓存&quot;">​</a></h4><p>如果计算一个散列值比较耗时，那么我可以这个计算好的值缓存起来，即使用一个变量把这个值保存起来，在下一次调用时直接返回。Java中的String就是采用的这种方式。</p><h2 id="拉链式的散列表" tabindex="-1">拉链式的散列表 <a class="header-anchor" href="#拉链式的散列表" aria-label="Permalink to &quot;拉链式的散列表&quot;">​</a></h2><p>散列函数能够将键值转换成数组的下标；第二步就是需要处理碰撞冲突，也就是需要处理遇到了两个散列值相同的对象应该如何存储。有一种方式是数组中的每一个元素都指向一个链表用来存放散列值相同的对象，这种方式被称为拉链法</p><p>拉链法可以使用原始的链表保存键，也可以采用我们之前实现的红黑树来表示，在java8中采用的这两种方式的混合，在节点数超过了8之后变为红黑树。</p><p>这里我们就采用简单的链表来实现拉链式散列表，数据结构使用在前几篇中已经实现的<code>LinkedMap</code>，可以参考前面的文章<a href="https://juejin.cn/post/6940442429229105183" target="_blank" rel="noreferrer">《基于数组或链表实现Map》</a>。</p><p><img src="https://cdn.jsdelivr.net/gh/silently9527/images/ff76067884aa4aa78b3221f598104621%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SeparateChainingHashMap&lt;K, V&gt; implements Map&lt;K, V&gt; {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div>`,53),i=[l];function t(c,h,o,r,d,u){return n(),s("div",null,i)}const v=a(e,[["render",t]]);export{b as __pageData,v as default};
