import{_ as s,c as n,o as a,aa as p}from"./chunks/framework.d_Ke7vMG.js";const m=JSON.parse('{"title":"17 图算法之计算图中最短路径","description":"","frontmatter":{"title":"17 图算法之计算图中最短路径","author":"Herman","updateTime":"2024-07-16 21:34","desc":"图算法系列之计算图中最短路径","categories":"算法","tags":"数据结构/广度优先搜索/图算法","outline":"deep"},"headers":[],"relativePath":"Notes/No2TechColumn/04 Algorithms/16 图算法系列之计算图中最短路径.md","filePath":"Notes/No2TechColumn/04 Algorithms/16 图算法系列之计算图中最短路径.md","lastUpdated":1723563552000}'),e={name:"Notes/No2TechColumn/04 Algorithms/16 图算法系列之计算图中最短路径.md"},l=p(`<h1 id="图算法系列之计算图中最短路径" tabindex="-1">图算法系列之计算图中最短路径 <a class="header-anchor" href="#图算法系列之计算图中最短路径" aria-label="Permalink to &quot;图算法系列之计算图中最短路径&quot;">​</a></h1><h2 id="前言" tabindex="-1">前言 <a class="header-anchor" href="#前言" aria-label="Permalink to &quot;前言&quot;">​</a></h2><p>在前面两篇中我们通过深度优先搜索可以从图中找出一条通过顶点v到顶点w的路径，但是深度优先搜索与顶点的输入有很大的关系，找出来的路径也不一定是最短的，通常情况下我们很多时候需要找出图中的最短路径，比如：地图功能。这里我们就需要使用到广度优先搜索算法</p><h2 id="广度优先搜索" tabindex="-1">广度优先搜索 <a class="header-anchor" href="#广度优先搜索" aria-label="Permalink to &quot;广度优先搜索&quot;">​</a></h2><p>依然使用之前定义的寻找路径的API</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class Paths {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>在广度优先搜索中，为了找出最短路径，我们需要按照起点的顺序来遍历所有的顶点，而不在是使用递归来实现；算法的思路：</p><ol><li>使用队列来保存已经被标记过但是邻接表还未被遍历过的顶点</li><li>取出队列中的下一个顶点v并标记它</li><li>将v相邻的所有未被标记的顶点加入到队列</li></ol><p>在该算法中，为了保存路径，我们依然需要使用一个边的数组edgeTo[]，用一颗父链树来表示根节点到所有连通顶点的最短路径。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class BreadthFirstPaths {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>以下图为列，来看看广度优先搜索的运行轨迹</p><p><img src="https://raw.githubusercontent.com/silently9527/images/main/0965db8ae1cd4b84ab7453e31c768db1%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p><img src="https://raw.githubusercontent.com/silently9527/images/main/e8c5b8f000754f0f8bb3027dc531b2c0%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p>单元测试的代码：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Test</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><p>执行的结果与我们分析的运行轨迹一致</p><p><img src="https://raw.githubusercontent.com/silently9527/images/main/ccd3716f1c514baf9238b8fa83300075%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><h2 id="符号图" tabindex="-1">符号图 <a class="header-anchor" href="#符号图" aria-label="Permalink to &quot;符号图&quot;">​</a></h2><p>最近几篇文章一起学习到的图算法都是以数字作为了顶点，是因为数字来实现这些算法会非常的简洁方便，但是在实际的场景中，通常都是使用的字符作为顶点而非数字，比如：地图上的位置、电影与演员的关系。</p><p>为了满足实际的场景，我们只需要在数字与字符串的关系做一个映射，此时我们会想到之前文章实现的map(通过二叉树实现map、红黑树实现map、哈希表实现map等等，有兴趣的同学可以去翻翻)，使用Map来维护字符串和数字的映射关系。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SymbolGraph {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>实现的思路：</p><ol><li>使用Map来保存字符串-数字的映射，key为字符串，value为数字</li><li>使用一个数组来反向映射数字-字符串，数组的下标对应数字顶点，值对应字符串名称</li></ol><p>假设构造图的顶点与边是通过字符串来表示的，如：<code>a,b,c,d\\nb,a,h,l,p\\ng,s,z</code> 使用\\n分隔的每段第一个字符串表示顶点v，后面的表示与顶点v相连的相邻顶点；</p><blockquote><p>实际的过程可以根据具体情况来确定，不一定非得这种字符串，可以来源于数据库，也可以来源于网路的请求。</p></blockquote><p>代码实现如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SymbolGraph {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div>`,27),i=[l];function t(c,r,o,h,d,g){return a(),n("div",null,i)}const b=s(e,[["render",t]]);export{m as __pageData,b as default};
