import{_ as s,c as n,o as a,aa as p}from"./chunks/framework.DOnJscRE.js";const v=JSON.parse('{"title":"15 图算法之深度优先搜索二","description":"","frontmatter":{"title":"15 图算法之深度优先搜索二","author":"Herman","updateTime":"2024-07-16 21:34","desc":"图算法系列之深度优先搜索二","categories":"算法","tags":"数据结构/深度优先搜索/图算法","outline":"deep"},"headers":[],"relativePath":"Notes/No2TechColumn/04 Algorithms/15 图算法系列之深度优先搜索二.md","filePath":"Notes/No2TechColumn/04 Algorithms/15 图算法系列之深度优先搜索二.md","lastUpdated":1724595807000}'),l={name:"Notes/No2TechColumn/04 Algorithms/15 图算法系列之深度优先搜索二.md"},e=p(`<h1 id="图算法系列之深度优先搜索-二" tabindex="-1">图算法系列之深度优先搜索（二） <a class="header-anchor" href="#图算法系列之深度优先搜索-二" aria-label="Permalink to &quot;图算法系列之深度优先搜索（二）&quot;">​</a></h1><p>在上篇中我们学习了深度优先搜索，知道了如何通过深度优先搜索在图中寻找路径；本篇我们继续一起来学习深度优先搜索算法的其他应用场景</p><h2 id="连通分量" tabindex="-1">连通分量 <a class="header-anchor" href="#连通分量" aria-label="Permalink to &quot;连通分量&quot;">​</a></h2><p>从一幅图中找出所有的连通分量，这是也是深度优先搜索的一个应用场景。什么是连通分量？这个定义在之前的文章中已有提到<a href="https://juejin.cn/post/6930395454739841037" target="_blank" rel="noreferrer">《如何检测社交网络中两个人是否是朋友关系（union-find算法）》</a></p><p>在这篇采用的是union-find算法实现的连通性检查，本篇我们将采用深度优先搜索的方式来找出图中的所有连通分量</p><h4 id="连通分量的api定义" tabindex="-1">连通分量的API定义 <a class="header-anchor" href="#连通分量的api定义" aria-label="Permalink to &quot;连通分量的API定义&quot;">​</a></h4><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class DepthFirstCC {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><h4 id="单元测试" tabindex="-1">单元测试 <a class="header-anchor" href="#单元测试" aria-label="Permalink to &quot;单元测试&quot;">​</a></h4><p><img src="https://cdn.jsdelivr.net/gh/silently9527/images/d5dd95fdc26242ffaacd395891193375%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p>构造这样一个图，连通分量的总数应该是3</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Test</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p><img src="https://cdn.jsdelivr.net/gh/silently9527/images/d59c6123ec994ac9b55c65464f561c12%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><blockquote><p>基于深度优先搜索实现的连通性检查理论上说要比以前实现的union-find算法更快，因为检查连通性深度优先搜索实现的版本能够保证在常量时间内完成，而union-find算法不行；</p><p>但是union-find也有自己的优势: 不需要把完整的构造并表示一张图，更重要的是union-find算法是动态的添加节点。</p></blockquote><h2 id="检查无向图中是否有环" tabindex="-1">检查无向图中是否有环 <a class="header-anchor" href="#检查无向图中是否有环" aria-label="Permalink to &quot;检查无向图中是否有环&quot;">​</a></h2><p>为了减小实现的复杂度，我们假设图中不存在自环和平行边；</p><p>假如从顶点v出发存在环，表示从顶点v出发的连通分量中某个顶点的邻接顶点是v，那么在搜索的过程中必定会再次遇到顶点v</p><p>实现的思路：</p><ol><li>标记已经搜索过的每个顶点</li><li>当遇到了一个已经被标记过的顶点，表示已经图中存在环；</li><li>由于图是无向图，如果v-w相连，那么顶点v中的邻接表中有w，w邻接表中也会有v，但是他们没有构成环，所以需要排除掉该情况。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class Cycle {</span></span>
<span class="line"><span>    private boolean marked[];</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>方法dfs的参数v表示需要待搜索的顶点，pV表示的是到达v的顶点，所以如果v的邻接表中有个顶点已被标记过并且该顶点不等于到达v的顶点，那么表示图中有环</p><h2 id="检查无向图是否是二分图" tabindex="-1">检查无向图是否是二分图 <a class="header-anchor" href="#检查无向图是否是二分图" aria-label="Permalink to &quot;检查无向图是否是二分图&quot;">​</a></h2><p>何为二分图? 图中每条边所连接的顶点都属于不同的部分；如下图：</p><p><img src="https://cdn.jsdelivr.net/gh/silently9527/images/e41927a074134d0c8782416244bef414%7Etplv-k3u1fbpfcp-zoom-1.image" alt=""></p><p>其中红色节点表示一个集合，白色节点是另一个集合，每条边连接的两个顶点属于不同的集合；</p><p>举个实际的例子就很好理解，电影与演员的关系，电影作为一个顶点，演员作为一个顶点，电影与电影直接是不会有边，演员与演员直接也不会有边，这就是一张二分图。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class TwoColorGraph {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div>`,32),i=[e];function c(t,o,r,d,h,u){return a(),n("div",null,i)}const b=s(l,[["render",c]]);export{v as __pageData,b as default};
