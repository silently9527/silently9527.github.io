import{_ as s,o as n,c as e,R as a,k as p}from"./chunks/framework.412fhWlr.js";const b=JSON.parse('{"title":"图解堆排序","description":"","frontmatter":{"title":"图解堆排序","date":"2021-08-14T13:59:24.511Z","updated":"2021-08-15T12:00:03.509Z","url":"/?p=48","categories":"算法","tags":"排序算法 | 堆排序"},"headers":[],"relativePath":"article/pamphlet/algorithms/基础算法/图解堆排序.md","filePath":"article/pamphlet/algorithms/基础算法/图解堆排序.md","lastUpdated":1723371853000}'),i={name:"article/pamphlet/algorithms/基础算法/图解堆排序.md"},l=a(`<blockquote><p>程序员常用的IDEA插件：<a href="https://github.com/silently9527/ToolsetIdeaPlugin" target="_blank" rel="noreferrer">https://github.com/silently9527/ToolsetIdeaPlugin</a></p><p>完全开源的淘客项目：<a href="https://github.com/silently9527/mall-coupons-server" target="_blank" rel="noreferrer">https://github.com/silently9527/mall-coupons-server</a></p><p>微信公众号：贝塔学Java</p></blockquote><h2 id="前言" tabindex="-1">前言 <a class="header-anchor" href="#前言" aria-label="Permalink to &quot;前言&quot;">​</a></h2><p>在上一篇中我们一起使用二叉堆实现了优先级队列，假如我们从构建好的优先级队列中持续调用删除最小（或者最大），把结果输出到另一个数组中，那么就可以把数组的所有元素进行排序，这就是本篇我们需要学习的<strong>堆排序</strong>。在看本篇之前需要先看下前一篇《原来实现优先级队列如此简单》</p><p>堆排序的过程主要有两个阶段：</p><ul><li>把原始数据构造成一个有序堆（构造堆）</li><li>从堆中按照递减顺序取出所有元素就可以得到排序结果（下沉排序）</li></ul><p>开始之前，我们需要把上一篇中的sink()方法修改一下；</p><p>保证我们在进行排序的时候直接使用原始的数组，无需建立一个辅助数组浪费空间；由于我们需要动态的缩小堆的大小，需要把size通过参数传入；</p><p>其次我们数组的下标是从0开始的，与之前的优先级排序有些差别，<strong>对于k节点的左边节点下标是2k+1，右边下标是2k+2</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code"><code><span class="line"><span>private void sink(Comparable[] queue, int k, int size) {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div>`,7),o=[l,t,r];function c(h,d,u,_,g,m){return n(),e("div",null,o)}const v=s(i,[["render",c]]);export{b as __pageData,v as default};
